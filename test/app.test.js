...
describe('Graphbook application test', function() {
  ...
  describe('authentication', function() {
    ...
    it('allows the user to sign up', function(done) {
      const json = {
        operationName: null,
        query:
          'mutation signup($username: String!, $email : String!, $password : String!) { signup(username: $username, email: $email, password : $password) { token }}',
        variables: {
  ...
  describe('frontend', function() {
    it('renders and switches to the login or register form', function(done) {
      ...
      wrapper
        .find('LoginRegisterForm')
        .find('a')
        .simulate('click');
      expect(wrapper.html()).to.contain(
        '<a>Want to login? Click here</a>',
      );
      done();
    });
